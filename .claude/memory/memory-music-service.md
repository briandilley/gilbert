...
`_extract_auth_code` parses whatever the user pasted — a full redirect URL (`http://localhost:8888/callback?code=…`), a query fragment (`?code=…`), or a bare code.
...
- `redirect_uri` — must match one registered on the Spotify app; default `http://localhost:8888/callback` (non-functional endpoint — the paste-code flow doesn't need it to respond).
