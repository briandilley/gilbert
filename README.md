...
- **Coding-agent conduit** — the `code-conduit` plugin makes Gilbert a *bidirectional messenger* between the user (typically over voice via the Mentra glasses) and a coding agent the user runs on their own machine — backends for [OpenCode](https://github.com/sst/opencode) (via `opencode serve`) and [Claude Code](https://docs.anthropic.com/en/docs/claude-code) (via `claude -p` subprocess + stop-hook webhook). Gilbert never writes or reviews code — it just relays messages verbatim in both directions. **Outbound**: "tell Claude to add error handling to the auth flow" → fire-and-forget POST/spawn → "Sent to opencode on the gilbert project". **Inbound**: SSE consumer (OpenCode) or `POST /api/code-conduit/inbound` webhook (Claude Code stop hook, CI scripts, anything else) → ring buffer + `code.notification` bus event + Gilbert notification with severity-based urgency (errors/attention → urgent sound + push, done → badge bump, info → silent). AI tools `code_send` (fire a message) and `code_recent_activity` (summarize what the agent's been doing) cover the conversational surface; slash equivalents `/code send` and `/code recent`; the `/coding` SPA page shows the same feed as a two-column view with a compose form. Operator-configured project aliases let the user say "the gilbert project" instead of pasting absolute paths.
