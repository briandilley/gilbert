...
- **Coding-agent conduit** — the `code-conduit` plugin makes Gilbert a *messenger* between the user (typically over voice via the Mentra glasses) and a coding agent the user runs on their own machine — currently [OpenCode](https://github.com/sst/opencode) via its `opencode serve` HTTP API; Claude Code planned. Gilbert never writes or reviews code — it just relays messages verbatim ("tell Claude to add error handling to the auth flow" → fire-and-forget POST to OpenCode → "Sent to opencode on the gilbert project"). The `code_send` AI tool fires the relay mid-conversation; the slash equivalent is `/code send <message> [project=<alias>]`. Operator-configured project aliases let the user say "the gilbert project" instead of pasting absolute paths. Outbound only in v1; inbound notifications (the agent finishes / errors / needs clarification → Gilbert surfaces via TTS) land in Phase 2.
