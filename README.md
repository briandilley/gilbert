...
- **Music and speaker control** — the `sonos` plugin discovers Sonos speakers on the LAN, handles playback/volume/grouping, and uses Spotify's Web API for browse/search. The Music service exposes search, queue, station ("play more like this"), and loop/repeat tools — capabilities are gated per-backend so swappable backends only surface what they actually support. Personal playlists (`/music playlist-create`, `-list`, `-show`, `-add`, `-remove`, `-update`, `-delete`, `-play`) are Gilbert-owned and per-user: they live in Gilbert's own storage rather than the upstream music service, so each household member curates their own without touching anyone else's Spotify account.
...
- **Scheduler (full cron)** — every schedule is a single cron expression evaluated by one engine. Supports 5-field POSIX (`30 3 * * *`), 6-field with seconds (`*/30 * * * * *`), ranges/lists/steps, `JAN`-`DEC` and `SUN`-`SAT` names, the Quartz day specifiers `L` / `W` / `#` (`0 0 * * FRI#3` = third Friday), the `@daily`-style macros, `@every 90s` for arbitrary intervals, and `@reboot` / `@once+45s` for one-shots. Schedules are timezone-aware with deliberate DST semantics (a job at a nonexistent spring-forward time fires once at the transition; an ambiguous fall-back time fires only on its first occurrence). Per-job **catch-up** policy decides what happens to fires missed while Gilbert was down (skip / one make-up run / backfill), and per-job **overlap** policy decides what happens when a fire is due while the previous one still runs. The `/scheduler` page shows each job's expression, next run, and policies.
