...
- **Bidirectional text messaging** — the `messaging` plugin (carrier-agnostic `MessagingService`) sends and receives SMS / MMS via a chosen `MessagingBackend` (the `telnyx` plugin contributes one). Inbound messages route to the configured owner-user and persist as threads; the `/messages` SPA page renders them as a familiar two-pane conversation view with a live compose box. Optional `auto_reply` flag routes inbound messages through `AIService.chat()` so Gilbert can text back on the user's behalf — tagged with `source="messaging"` so the AI's history doesn't pollute the chat sidebar. Slash command `/msg send <number> <body>`; AI tool `send_text_message`. Webhook lands at `/api/telnyx/messages/webhook`.
