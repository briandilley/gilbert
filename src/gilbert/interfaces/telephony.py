...
# ── Telnyx webhook + media-stream endpoint capability ───────────────
#
# The core ``/api/telnyx/*`` routes need to hand inbound webhooks and
# bidirectional media frames to whichever plugin currently owns the
# Telnyx integration. Rather than reach across the layer boundary and
# import the plugin module directly, the route resolves the
# ``telnyx_webhook`` capability and dispatches through these protocols.
# Any future Telnyx-compatible plugin can satisfy them.


@runtime_checkable
class TelnyxMediaSession(Protocol):
    """One in-flight Telnyx call as seen by the media-stream WS route.

    The route only reads/writes a handful of fields; the rest of the
    session lifecycle stays inside the plugin.
    """

    call_id: str
    stream_id: str
    media_ws: object  # set/cleared by the route; the plugin doesn't import FastAPI

    async def push_audio_in(self, chunk: bytes) -> None: ...


@runtime_checkable
class TelnyxWebhookEndpoint(Protocol):
    """Capability exposed by the Telnyx plugin so the core
    ``/api/telnyx/*`` routes can dispatch webhooks and bind media-stream
    websockets to the right session without importing the plugin
    module directly.
    """
