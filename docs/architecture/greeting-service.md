...
## Related
- [Weather Service](weather-service.md) — provides the `WeatherProvider` capability that feeds the weather blurb.
- `WeatherProvider` is the capability protocol used to access weather without coupling to the concrete service class.
- Both `style` and `weather_hint_template` ship with `ai_prompt=True` since their values flow into the greeting AI prompt.
