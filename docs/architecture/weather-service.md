...
## Related
- Backend Pattern — universal ABC + registry pattern this feature follows.
- Multi-backend Aggregator Pattern — service holds backends; the cache key already includes `backend_name` to support a future per-method multi-backend layout.
- Capability Protocols — `WeatherProvider` belongs in `interfaces/`.
- AI Prompts Are Always Configurable — `weather_hint_template` on `GreetingService` ships with `ai_prompt=True` because its rendered output is interpolated into the AI greeting prompt as context.
- Multi-User Isolation — singleton-safe state, fresh `storage.get` per call, no per-user prefs cache on `self`.
- Scheduler Service — `weather.digest` and `weather.alerts.poll` system jobs.
- Notification Service — used by severe-alert delivery.
- Slash Commands — `slash_group="weather"` collapse + the AI-visible / slash-only distinction for `set_home_location` / `set_units`.
- Web Search Service — closest existing analog.
