...
- Multi-user isolation — singleton + concurrent users + per-user keyed state.
- Backend pattern — the universal registry pattern.
- Capability protocols — how greeting + proposals consume `HealthProvider`.
- User and authentication system — `auth.user.deleted` event publisher.
- AI prompts configurability — the `summary_prompt` / `trend_prompt`
  configurability rule.
- UI blocks — `confirm_or_execute` two-step delete wizard.
